					...
					<div class="col-xs-12 col-md-12 col-lg-12">
						<div class="panel panel-primary">
							...
							<div class="panel-body">
								...
								<form action="{{url('DSMonAn/SuaMonAn')}}" method="post" enctype="multipart/form-data">
									<input type="hidden" name="_token" value="{{csrf_token()}}">
									@foreach ($SanPham as $key)
									<input type="hidden" name="id_sanpham" value="{{$key->id_sanpham}}">
									<div class="row" style="margin-bottom:40px">
										<div class="col-xs-8">
											...
											<div class="form-group" >
												...
												<input required type="text" name="avt_sanpham" class="form-control" value="{{$key->avt_sanpham}}" >
											</div>
											<div class="form-group" >
												<label>Tên Mon An</label>
												<input required type="text" name="name_sanpham" class="form-control" value="{{$key->name_sanpham}}" >
											</div>
											<div class="form-group" >
												<label>Giá</label>
												<input required type="text" name="gia" class="form-control" value="{{$key->gia}}" >
											</div>
											@endforeach
											<div class="form-group" >
												...
												<select required name="theloai" class="form-control">
													...
													<option value="{{$key->id_theloai}}">{{$key->theloai}}</option>
													@endforeach	
												</select>	
											</div>	
											<div class="form-group" >
												<label>Tên Địa điểm</label>
												<select required type="text" name="id_diadiem" class="form-control" >
													@foreach ($DiaDiem as $key)
													<option value="{{$key->id_diadiem}}">{{$key->ten}}</option>
													@endforeach
												</select>
											</div>							
